...
- Detailed DexScreener pair/liquidity diagnostics enabled.
- Detailed Jupiter per-token confirmation diagnostics enabled.
  */
...
/* ============================================================
BEST DEX PAIR — DETAILED DIAGNOSTIC VERSION
============================================================ */
...
let selectionStatus =
  "SELECTED_HIGHEST_LIQUIDITY";

if (
  selectedLiquidity <= 0
) {
  selectionStatus =
    "SELECTED_PAIR_ZERO_OR_MISSING_LIQUIDITY";
} else if (
  selectedLiquidity <
  MIN_LIQUIDITY_USD
) {
  selectionStatus =
    "SELECTED_PAIR_BELOW_MIN_LIQUIDITY";
}
...
return {
  ...
  diagnostic: {
    ...
    highest_liquidity_usd:
      Math.max(
        ...liquidityValues
      ),

    lowest_liquidity_usd:
      Math.min(
        ...liquidityValues
      ),
...
"${JUPITER_PRICE_API}?ids=${encodeURIComponent( batch.join(",") )}";
...
for (
  ...
  if (
    diagnostics.price_samples.length <
    10
  ) {
...
total_solana_pairs_returned: 0,

zero_liquidity_pairs: 0,
below_min_liquidity_pairs: 0,

failure_reasons: {},

pair_selection_samples: [],
samples: []
...
if (
  ...
  hydrationDiagnostics.total_pairs_returned +=
    safeNumber(
      pairDiagnostic.total_pairs
    );

  hydrationDiagnostics.total_solana_pairs_returned +=
    safeNumber(
      pairDiagnostic.solana_pairs
    );

  if (
    pairDiagnostic.selection_status ===
    "SELECTED_PAIR_ZERO_OR_MISSING_LIQUIDITY"
  ) {
    sourceCounts.dex_zero_liquidity_pairs++;
    hydrationDiagnostics.zero_liquidity_pairs++;
  }
  ...
  if (
    ...
    hydrationDiagnostics.below_min_liquidity_pairs++;
  }

  if (
    hydrationDiagnostics.pair_selection_samples.length <
    10 &&
    (
    ...
    hydrationDiagnostics.pair_selection_samples.push(
      pairDiagnostic
    );
  }
}
...
if (
  ...
  if (
    ...
    if (
      jupiterDiagnostics.price_samples.length <
      20
    ) {
      jupiterDiagnostics.price_samples.push(
        sample
      );
    }
...
deltas: {
  ...
  change_1h: null,
  volume_1h: null,
  buy_pressure_5m: null,
  buy_pressure_1h: null,
  liquidity: null
}
